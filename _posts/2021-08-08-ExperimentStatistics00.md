...
눈금값 오차도 표본평균, 표본표준편차, 표준오차와 별개로 결과값에 명시해야한다는 것으로 이 모든 값을 고려한 측정값의 대표 표기는 다음과 같이 해야 할 것입니다. (표본평균=$$5.18$$, 측정 횟수=36, 표본표준편차=$$0.024$$, 눈금값=$$0.05$$ 이며, 표준오차는 $$0.024/\sqrt{36}=0.004$$)

$$5.18 (\pm 0.004) \pm 0.024\mathrm{(stat.)} \pm 0.05\mathrm{(sys.)}$$
