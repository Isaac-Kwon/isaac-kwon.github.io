...
<!-- > ~~아니, 하드한 통계학을 하면 할 수는 있습니다. Unfolding Process with Bayesian Statistics~~ -->
...
위 그림은, $$5$$ 를 평균으로 하고 $$2$$ 를 표준편차로 하는 정규분포를 모분포로 하는 난수 생성기로부터, $$N$$개의 실수를 무작위 추출하여 평균을 구한 것을 변량으로 하는 히스토그램입니다. ($$5\mathrm{mm}$$ 가 측정값 모분포의 평균이고, $$2\mathrm{mm}$$ 가 표준편차인 측정환경으로부터 측정한 값 $$N$$ 개를 추출하여 평균을 취한것과 동일합니다.) 여기서의 "*$$N$$개의 실수를 무작위 추출한 평균*" 은 통계학에서 "**표본평균**" 이라고 부릅니다. 그리고 *그것과 함께 구할 수 있는 표준편차*는 "**표본표준편차**" 라고 부릅니다.
...
아직, 눈금값 오차를 고려하지 않았습니다. 위의 평균값 계산값과 $$5.02$$ 와의 차이인 $$0.00133$$ 은, 눈금값오차로 입력한 $$k=0.1$$ 보다 작습니다.

눈금값 오차도 표본평균, 표본표준편차, 표준오차와 별개로 결과값에 명시해야한다는 것으로 이 모든 값을 고려한 측정값의 대표 표기는, 통계값이 늘어날수록 줄어드는 통계적 불확도와 통계가 늘어남에 따라 영향을 받지 않는 계통적 불확도로 분리하여 다음과 같이 표기해야 할 것입니다. (표본평균=$$5.18$$, 측정 횟수=36, 표본표준편차=$$0.024$$, 눈금값=$$0.05$$ 이며, 표준오차는 $$0.024/\sqrt{36}=0.004$$)

$$5.18 \pm 0.004{(stat.)} \pm 0.05\mathrm{(sys.)} \mathrm{~~with~} N=36 $$

보통의 경우에는, 통계적 불확도 (위의 $$stat.$$)와 계통적 불확도(위의 $$sys.$$) 를 합성 $$(\sqrt{0.004^2 + 0.05^2}=0.05016)$$ 하여 표기하기 때문에, 
...
~~닭 잡는데 소 잡는 칼이긴 한데~~ 측정값이 만들어지는 과정을 아주 잘 알고 있고, 그것을 어떤 방식으로든 계산으로 잘 재현할 수 있을 때, Unfolding 이라는 과정을 쓰면 측정값의 분포로부터 참값에 더 가까이 다가갈 수 있습니다. 조만간 이에 대해서 포스트를 작성해보겠지만, 어떻게 될지는 잘 모르겠습니다. *To be continue...*