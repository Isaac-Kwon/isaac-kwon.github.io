...
$$
\begin{align}
0 &= \sum_n{ \vec{x_n}' \left(1 - \frac{\left(\hat{v_0}\cdot \vec{x_n}' \right) }{ \left(\vec{v_0}\cdot\vec{v_0}\right)^{1/2}} \right) } \\
0 &= \sum_n{  \left(\hat{v_0}\cdot\vec{x_n}'\right) \left( 1 + \frac{\left(\hat{v_0} \cdot \vec{x_n}' \right) }{ \left(\vec{v_0}\cdot\vec{v_0}\right)^{1/2} } \right) } 
\end{align}
$$

의 서로 유사해보이는 두 식으로 분리할 수 있습니다. 두번째 식에는, 맨 앞에 $$\hat{v_0}$$ 이 붙어이었어야 하나 값의 정의상 0이 될 수 없기 때문에 그 뒤 급수만 표기하였습니다.
